(function($, pluginNumber) {
  ...
  function summryInfo() { // count and make lower status string  
    $('span.infoField').text(' total: ' + $(liInputs).length + ' done: ' + $(liInputs + ':checked').length);
  ...
  function addTask(ev) {
    ...
    if (textForNewItem.length)
      $('<li>', {
        text: textForNewItem,
        class: 'bounceInDown animated roundedTwo'
      })
      .css({
    ...
    $('input[name="bjattaToDoListInputTextField"]').val('');
  }

  function pulseOff(el) {
    $(el).removeClass(typeOffAnimatedChecked);
  }

  function pulseOn(el) {
    $(el).addClass(typeOffAnimatedChecked);
  }
  ...
  function deleteOff() {
    ...
    for (var i = checkedLi.length - 1; i >= 0; i--) {
      setTimeout(function(el) {
        $(el).addClass(typeOffAnimatedDelete);
      }, i * 150, checkedLi[i]);
      setTimeout(function(el) {
        $(el).remove();
        summryInfo();
      }, (i - 1) * 160 + animationTime * .9, checkedLi[i])
    }
  }
  ...
  $('<div>', {
      class: 'bjattaToDoListMain ui-draggable ui-draggable-handle',
  ...
  $('<div>', {
    ...
    .append($('<ul>', {
      name: 'bjattaToDoListUl',
      class: 'bjattaToDoList',
      id: 'bjattaToDoListUl',
    }));

  $('<span>', {
      'name': 'bjattaToDoListInfoField',
      class: 'infoField'
    })
    .appendTo('div[name="bjattaToDoListMainDiv"]');
